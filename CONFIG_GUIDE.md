...
1. 先替换插件 DLL 和依赖 DLL
2. 保留现有 `config.yml` / `translations.yml`
3. 启动服务器一次
4. 看日志里有没有 CrimsonSon 配置加载失败的警告
5. 如果没有警告，再打开配置文件补新字段或微调旧值
6. 如果有警告，先备份旧文件，再删除出问题的那一份，让它自动重生
7. 用新生成的文件对照旧文件，把你真正需要保留的改动手动合回去
