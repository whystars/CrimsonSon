...
`1.2.0` 调整了配置结构。升级时请先备份并删除旧版 `config.yml` 和 `translations.yml`，启动服务器让插件重新生成，再把需要保留的自定义数值和文本迁移回新文件。

旧配置中的 `CassieAnnouncementWords` 已废弃；新版使用 `CassieBroadcastHoldSeconds` 控制 CASSIE 字幕延时，并新增 `CountDummySpectatorsForSummon` 控制 dummy 旁观者是否计入召唤候选。
